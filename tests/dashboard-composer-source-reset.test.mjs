...
test("onSend resets composerSource after clearing the composer", () => {
  ...
  const end = SRC.indexOf("const addFiles = useCallback", start);
  assert.notEqual(start, -1, "located onSend");
  assert.notEqual(end, -1, "located the end of onSend");
...
test("scheduleSend resets composerSource after clearing the composer", () => {
  ...
  const end = SRC.indexOf("const cancelScheduledSend = useCallback", start);
