...
// ── summary quality audit (2026-07-04): real-transcript failures ─────────
test("the reconnect meta-question is banned from what_they_want output", () => {
  const source = readFileSync(
    fileURLToPath(new URL("../apps/runner/dist/services/ai.js", import.meta.url)),
    "utf8"
  );
  // The old guidance told the model to 'Frame as: "what's the warmest..."'
  // and the model copied the meta-question verbatim into summaries on
  // ACTIVE threads. The prompt must now ban that output shape explicitly.
  assert.match(source, /BANNED OUTPUT: never write the meta-question itself/);
  assert.doesNotMatch(
    source,
    /Frame as: "what's the warmest/,
    "the copyable 'Frame as' meta-question phrasing must stay deleted"
  );
});

test("nothing-pending is first-class and old arcs cannot become manufactured jobs", () => {
  const source = readFileSync(
    fileURLToPath(new URL("../apps/runner/dist/services/ai.js", import.meta.url)),
    "utf8"
  );
  assert.match(source, /"NOTHING PENDING" IS A FIRST-CLASS ANSWER/);
  assert.match(source, /Manufactured homework is worse than an honest nothing-pending/);
  // Quote-the-question + proper-noun discipline (Rolanda / Power Must Change
  // Hands regressions).
  assert.match(source, /CARRY THE CONTACT'S ACTUAL WORDS/);
  assert.match(source, /Power Must Change Hands/);
  // Invented specifics ban (invented "tonight"/"next Sunday" regressions).
  assert.match(source, /NEVER INVENT SPECIFICS/);
  // Required points must trace to a live message (fabricated meetup regression).
  assert.match(source, /TRACEABILITY: every required point must trace to a specific message/);
});

test("the code derives the latest unanswered inbound run and feeds it as authoritative grounding", () => {
  const source = readFileSync(
    fileURLToPath(new URL("../apps/runner/dist/services/ai.js", import.meta.url)),
    "utf8"
  );
  // Deterministic grounding beats prompt-hope: the code slices the contact's
  // messages since the operator's last outbound and the prompt block names
  // them as the authoritative source for what_they_want.
  assert.match(source, /LATEST UNANSWERED INBOUND RUN \(code-derived/);
  assert.match(source, /unansweredRunBlock/);
  // The block must be injected into the assembled prompt.
  assert.ok(source.includes("${unansweredRunBlock}"), "block must be template-injected");
  // Empty-ask guard: a whitespace-only what_they_want falls back to the
