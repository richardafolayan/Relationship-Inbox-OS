...
// Light, human labels. Avoids task-manager / CRM language on purpose — these
// are reply prompts, not tickets.
export const CATEGORY_LABEL: Record<ActionItemCategory, string> = {
  ...
  "follow-up": "Follow-up",
  update: "Update",
  mention: "They mentioned"
};

/**
 * Lightweight, deterministic categorisation of an open-loop string.
 *
 * Heuristic only — no AI call, no schema, no stored category. Checks run in
 * priority order and the first match wins, so a question that also reads like
 * a request is still labelled "question". Returns "mention" when nothing
 * matches, which renders as a neutral "They mentioned" tag.
 */
export function categorizeActionItem(text: string): ActionItemCategory {
  const t = text.trim().toLowerCase();
  if (!t) return "mention";

  // A trailing "?" is the strongest signal. The leading-word check is kept
  // to interrogatives and question auxiliaries — modal verbs (can / could /
  // would) are left out, since "can you send..." reads as a request unless
  // it actually ends in "?".
  const isQuestion =
    t.endsWith("?") ||
    /^(what|why|how|when|where|who|which|did|do|does|is|are|was|were)\b/.test(t);
  if (isQuestion) return "question";

  // Follow-up is checked before request: an item explicitly about timing or
  // revisiting ("circle back next week", "check in later") stays a follow-up
  // even when it also carries request-ish words like "wants".
  if (
    /\b(follow up|followup|follow-up|check in|check-in|circle back|get back|next week|next month|later|reminder|remind|catch up)\b/.test(
      t
    )
  ) {
    return "follow-up";
  ...
  if (
    /\b(asked|asking|wants?|needs?|requested|can you|could you|would you|please|send (me|over|through|me the)|let me know|waiting (on|for)|owe|chase|reply with)\b/.test(
      t
    )
  ) {
    return "request";
  ...
  if (
    /\b(excited|worried|nervous|stressed|anxious|happy|sad|upset|frustrated|proud|gutted|buzzing|sorry|grateful|relieved|disappointed|overwhelmed|struggling)\b/.test(
      t
    )
  ) {
    return "feeling";
  ...
  if (
    /\b(mentioned|shared|said|told|update|news|announced|started|finished|launched|moved|landed|got the|got a|got an|just got)\b/.test(
      t
    )
  ) {
    return "update";
